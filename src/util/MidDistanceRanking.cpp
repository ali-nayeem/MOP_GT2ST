...
MidDistanceRanking::MidDistanceRanking (SolutionSet * solutionSet) {
  ...
  if (dominateMe == NULL) {
    cout << "Fatal Problem: Cannot reserve memory in class MidDistanceRanking" << endl;
    exit(-1);
  }
  ...
  if (ranking_ == NULL) {
    cout << "Fatal Error: Impossible to reserve memory in MidDistanceRanking" << endl;
    exit(-1);
  }
 ...
 * Destructor
 */
MidDistanceRanking::~MidDistanceRanking() {
 ...
 */
SolutionSet * MidDistanceRanking::getSubfront(int rank) {
 ...
 */
int MidDistanceRanking::getNumberOfSubfronts() {
